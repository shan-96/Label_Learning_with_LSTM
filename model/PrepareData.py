...
def prepare_train_csv(main_file_df):
    ...
    sample_df = sample_df['resolution'].apply(lambda x: '')
    sample_df.to_csv(path_or_buf=TRAIN_CSV_FILE, quoting=csv.QUOTE_NONE)
...
def prepare_test_csv(main_file_df):
    ...
    sample_df.to_csv(path_or_buf=TEST_CSV_FILE, quoting=csv.QUOTE_NONE)


def prepare_label_csv(main_file_df):
    split = int(len(main_file_df) * 0.1)
    sample_df = main_file_df.iloc[-split]
    sample_df.to_csv(path_or_buf=LF_CSV_FILE, quoting=csv.QUOTE_NONE)
...
class PrepareData:
    ...
    def prepare_data(self):
        ...
        main_file_df = main_file_df['resolution'].apply(lambda x: 1 if x in fixed_list else 0)
